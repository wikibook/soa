...
public class TypeConverter {
  ...
  public EBayCategory toEBayCategory(Category category) {
    ...
    default:
      throw new RuntimeException("Category " + category.name()
          + " can not be processed.  Additional runtime logic is "
          + "needed in the code.");
    }
  }
  ...
  public YahooShoppingTopLevelCategory toYahooShoppingTopLevelCategory(
      Category category) {
    if (category == null) {
      return null;
    }
  ...
  // ! <example xn="TypeConverter_toOfferType">
  ...
  public Picture toPicture(Image img) {
    ...
    return new Picture(picUrl, 
        toInteger(img.getWidth().getValue().toBigInteger()), 
        toInteger(img.getHeight().getValue().toBigInteger()));
  ...
  public Price toPrice(
    ...
    if ( p != null ) {
      ...
      if ( curStr == null ) { curStr = "USD"; }
      price = Price.getPrice(curStr, 
          Double.valueOf(p.getAmount().doubleValue()/100));
    }
    return price;
